...
def get_account(account_id: int, db: Session = Depends(get_db)):
    return account_service.to_account_read(_require_account(db, account_id))


@router.put("/{account_id}", response_model=AccountRead)
def update_account(account_id: int, data: AccountUpdate, db: Session = Depends(get_db)):
    account = account_service.update_account(db, _require_account(db, account_id), data)
    return account_service.to_account_read(account)
...
def deactivate_account(account_id: int, db: Session = Depends(get_db)):
    account = account_service.set_account_active(db, _require_account(db, account_id), False)
    return account_service.to_account_read(account)
...
def reactivate_account(account_id: int, db: Session = Depends(get_db)):
    account = account_service.set_account_active(db, _require_account(db, account_id), True)
    return account_service.to_account_read(account)
