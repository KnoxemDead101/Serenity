...
def test_create_account(client):
    response = client.post("/serenity-api/accounts", json=VALID_ACCOUNT)
...
def test_created_account_appears_in_list(client):
    client.post("/serenity-api/accounts", json=VALID_ACCOUNT)
    response = client.get("/serenity-api/accounts")
...
def test_get_one_account(client):
    client.post("/serenity-api/accounts", json=VALID_ACCOUNT)
    assert client.get("/serenity-api/accounts/1").status_code == 200
    assert client.get("/serenity-api/accounts/999").status_code == 404


def test_rejects_unknown_account_type(client):
    bad = {**VALID_ACCOUNT, "account_type": "Piggy Bank"}
    response = client.post("/serenity-api/accounts", json=bad)
    assert response.status_code == 422
    assert "Account type must be one of" in response.text


def test_rejects_blank_name(client):
    response = client.post("/serenity-api/accounts", json={**VALID_ACCOUNT, "name": "   "})
    assert response.status_code == 422


def test_rejects_fractions_of_a_cent(client):
    response = client.post("/serenity-api/accounts", json={**VALID_ACCOUNT, "opening_balance": "10.005"})
    assert response.status_code == 422


def test_options_endpoint(client):
    options = client.get("/serenity-api/accounts/options").json()
...
def test_dashboard_summary(client):
    client.post("/serenity-api/accounts", json=VALID_ACCOUNT)
    client.post("/serenity-api/accounts", json={**VALID_ACCOUNT, "name": "Biz", "classification": "Business",
